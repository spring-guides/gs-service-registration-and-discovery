...
to start up a registry with which other applications can communicate. This is a regular
...
Now that you have started a service registry, you can start up clients that interact with the registry. Our client applications, ServiceA and ServiceB, automatically register with the Eureka server because we have `spring-cloud-starter-netflix-eureka-client` on the classpath.
...
== Summary
Congratulations! You have just used Spring to start up a Netflix Eureka service registry
