...
You can use the IDE or execute the `./gradlew bootRun` command from each application folder.
...
- Run `eureka-server` with the `./gradlew bootRun` command from the `eureka-server` folder
- Run `servicea` with the `./gradlew bootRun` command from the `servicea` folder
- Run `serviceb` with the `./gradlew bootRun` command from the `serviceb` folder
